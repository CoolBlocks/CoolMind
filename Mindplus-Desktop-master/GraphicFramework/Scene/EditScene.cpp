...
void EditScene::saveFile(const QString &filePath)
{
	...
	query.exec("create table parameter("
			   "postfix text ,"	//SLJ add to be compliant the last officail release of Mind+
			   "name text ,"
			   "value text)");
...
void EditScene::createLine(const QString &startName,const QString &endName, const QPointF &startPos, const QPointF &endPos, GraphicsItemBase *pStartItem , GraphicsItemBase *pEndItem)
{
	if(!startName.isNull() && startPos != endPos)
	{
		...
		if(!path.isEmpty())
		{
			...
			qDebug() <<"startName:" << startName <<"endName:"<<  endName;	//SLJ
			qDebug() <<"tmpList1:" << tmpList1 <<"tmpList2:"<<  tmpList2;	//SLJ
			...
			qDebug() << "hashName:" << pMyLine->getName() <<" lineInfor.startName:endName" << lineInfor.startName << " : " << lineInfor.endName;	//SLJ
...
//寻路用的函数
//给定起点和终点, 返回适合的路径
QPainterPath EditScene::findPath(const QPointF &startPos_orig, const QPointF &endPos_orig, const QPointF &PathStartPos, const QPointF &PathEndPos)
{

	QPointF startPos = startPos_orig;	//SLJ
	QPointF endPos = endPos_orig;		//SLJ
...
//SLJ add begin
	if(startPos_orig.x()<PathStartPos.x())
		startPos.setX(startPos.x()-10);
	else if(startPos_orig.x()>PathStartPos.x())
		startPos.setX(startPos.x()+10);

	if(endPos_orig.x()<PathEndPos.x())
		endPos.setX(endPos.x()-10);
	else if(endPos_orig.x()>PathEndPos.x())
		endPos.setX(endPos.x()+10);

	qDebug() << "New 寻路起点!!!:" << startPos<<"寻路终点~~:" << endPos;
	qDebug() << "New 路径起点!!!:" <<PathStartPos << "路径终点~:" << PathEndPos;
//SLJ add end
...

{
				int yn = y+i;
				int xn = x+j;
				int endx = endNode_.x/10;
				//qDebug() << "### i:j" << i << j;

				//if( yn!=endNode_.y/10 && xn<endx && xn+1>=endx) continue;
				//if( yn!=endNode_.y/10 && xn>endx && xn-1<=endx) continue;

				if(i==-1) yn = y+i-1;
				else if(i==1) yn = y+i+1; 
					
				if(j==-1) xn = x+j-1;
				else if(j==1) xn = x+j+1;
				
				if( matrixNode_[yn][xn].type == TYPE_ROADBLOCK && matrixNode_[y+i][x+j].type != TYPE_END)
					continue;
}
#if 0
{
				int yn1 = min(y+i+1,MaxMatrixNodeY-1);
				int xn1 = min(x+j+1,MaxMatrixNodeX-1);

				int yn2 = max(y+i-1,0);
				int xn2 = max(x+j-1,0);
				
				qDebug() << "yn:xn" << yn1 << xn1 << "  " << yn2 << xn2;
				
				if( matrixNode_[yn1][x+j].type == TYPE_ROADBLOCK ||
					matrixNode_[y+i][xn1].type == TYPE_ROADBLOCK ||
					matrixNode_[yn2][x+j].type == TYPE_ROADBLOCK ||
					matrixNode_[y+i][xn2].type == TYPE_ROADBLOCK) // ||
					//matrixNode_[y+i][x+j].type  == TYPE_CLOSE)
				{
					continue;
				}
}
...
//SLJ add end
				...
				*如果保持openList按F值排序,改变后需要重新对openList排序
				...
				else if(TYPE_END == matrixNode_[y+i][x+j].type)
				{
					...
					m_points.append(QPointF(PathEndPos.x()+5, PathEndPos.y()-5));	//SLJ 平滑曲线
					m_points.append(QPointF(PathEndPos.x()+5, PathEndPos.y()-5));	//SLJ 平滑曲线
					...
					qDebug() << "m_points.size(), i : " << m_points.size() << i;
					
					while (i < m_points.size())
					{
						path.lineTo(m_points.at(i).x(), m_points.at(i).y());	//SLJ
						i ++;
					}
					
					/*path.cubicTo(m_points.at(i), m_points.at(i), m_points.at(i+1));	//SLJ
					i = i + 2;*/
...
//SLJ add begin
void EditScene::dumpMatrixNode()
{
	QString str,str1;
	qDebug("dumpMatrixNode():\n");
	for(int i = 0; i < 261; i++)
	{
		str.sprintf("  %03d:", i);
		//qDebug("  %03d:", i);
		for(int j = 0; j < 409; j++)
		{
			//qDebug("  %d", matrixNode_[i][j].type);
			str1.sprintf("  %d", matrixNode_[i][j].type);
			str = str + str1;
		}
		//qDebug("\n");
		qDebug() << str;
...
//SLJ modify being
				if(TYPE_ROADBLOCK != matrixNode_[yPos/MIN_CELL+i][xPos/MIN_CELL+j].type)
				{
					//qDebug() << "要设置障碍在点"  <<xPos/MIN_CELL+j <<","  << yPos/MIN_CELL+i;
					matrixNode_[yPos/MIN_CELL+i][xPos/MIN_CELL+j].type = TYPE_ROADBLOCK;
				}				
...
//SLJ Begin
	...
	LineSaveInfor lineInfo;
	lineInfo.startName = startName;
	lineInfo.endName = endName;

	QString keyName = hash_Name_LineSaveInfor.key(lineInfo);
	if(hash_Name_LineSaveInfor.contains(keyName))
	{
		hash_Name_LineSaveInfor.remove(keyName);
	}
	else
	{
		qDebug() << "删除Line的时候hash_Name_LineSaveInfor中没有找到对应的startName:endName " << startName << " : " << endName;
	}
...
//SLJ Begin
	...
	bool ifFindLine = false;
	 
	QHash<QString, LineSaveInfor>::iterator i;

	qDebug() << "###deleteLine()";
	for (i = hash_Name_LineSaveInfor.begin(); i != hash_Name_LineSaveInfor.end(); ++i)
    {
    	if( (i.value().startName == startName) && (i.value().endName == endName) )
    	{
			qDebug() << "hash_Name_LineSaveInfor.remove " << i.key();
    		hash_Name_LineSaveInfor.remove(i.key());
			ifFindLine = true;
			break;
    	}
	...
	if(!ifFindLine)
		qDebug() << "###Can't find line " << "startName:" << startName <<"endName:"<<  endName;
//SLJ End

#if 0
	foreach (LineSaveInfor lineInfo, hash_Name_LineSaveInfor)
	{
		if( (lineInfo.startName == startName) && (lineInfo.endName == endName) )
		{
			qDebug() <<"Find line, startName:" << startName <<" endName:"<< endName;
			ifFindLine = true;
		}
	}
#endif
	...
	if(startName == "start")
	{
		name = startName + tmpList2[0]+"_"+tmpList2[1];
	}
	else if(endName == "start")
	{
		name = tmpList1[0]+"_"+tmpList1[1] + endName;
	}
	else
	{
	...
	//hash_Name_LineSaveInfor.insertMulti(pMyLine->getName(), lineInfor);
	if(hash_Name_LineSaveInfor.contains(name))
	{
		hash_Name_LineSaveInfor.remove(name);
	}
	else
	{
		qDebug() << "删除Line的时候hash_Name_LineSaveInfor中没有找到对应的name: " << name;
	}
#endif
//SLJ End
}
 ...
 * \return 新创建的group的指针
 */
GraphicsItermGroup* EditScene::createItemGroup(const QPointF &pos, const QString &bbName1, int defaultId = -1, bool load = false, QString fullName = "", QString defaultValue = "")
{
	Q_UNUSED(defaultValue);
	//添加事物看看是不是能够有效改善速度
	QSqlDatabase::database().transaction();
	qDebug() << "坐标" <<pos;
    qDebug() << "要创建" << bbName1;   //SLJ

//SLJ begin
    QString bbName;
    QStringList paramInlist = bbName1.split(":");
	bbName = paramInlist.at(0);
	paramInlist.removeFirst();
    qDebug() << "###" << bbName<<paramInlist;
//SLJ end
	...
	for(int i = 0; i != bbData_.paramIn.size(); ++i)
	{
		...
		{
			value = paramInlist.at(i);
			qDebug() << "###paramIn " <<i<<value;
		}
		//SLJ end
