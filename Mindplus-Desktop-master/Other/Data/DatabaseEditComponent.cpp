...
void DatabaseEditComponent::updateParameter(const QString &type, const QString &value, const QString &ownerName)
{
    ...
    if(tr("INT") == type.toUpper())
    {
        ...
        {
            //cmd = tr("UPDATE Parameter SET Value = '%1' WHERE Owner = '%2'").arg(value.toInt()).arg(ownerName); //SLJ
            cmd = tr("UPDATE Parameter SET Value = '%1' WHERE Owner = '%2'").arg(value).arg(ownerName); //SLJ
        }
